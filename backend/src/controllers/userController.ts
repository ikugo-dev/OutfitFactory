...
async createUser(req: Request, res: Response): Promise<void>
{
    try {
        ...
        console.log(hashedPassword instanceof String);
...
async updatePassword(req: Request, res: Response) : Promise<void> 
{
    try {
        ...
        if (!(await Auth.verifyPassword(user.password, oldPassword))) {
            res.status(400).json({error: "Wrong password."}).send(); return;
        }
            
        const newHashedPass = await Auth.hashPassword(newPassword);
        const updateRes = await UserModel.updateOne({ _id: id }, {$set: { password: newHashedPass}}).exec();
        if (updateRes.modifiedCount == 0 ) { 
